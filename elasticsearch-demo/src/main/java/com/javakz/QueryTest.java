...
public class QueryTest {
    ...
    public void searchPaging() throws Exception {
        ...
        SearchResponse response = searchRequestBuilder.setQuery(QueryBuilders.matchAllQuery())
                .setFrom(2).setSize(2).execute().actionGet();
    ...
    public void searchAll() throws Exception {
        SearchRequestBuilder searchRequestBuilder = client.prepareSearch("film").setTypes("dongzuo");
        SearchResponse response = searchRequestBuilder.setQuery(QueryBuilders.matchAllQuery()).execute().actionGet();
        SearchHits searchHits = response.getHits();
        for (SearchHit searchHit:searchHits) {
            System.out.println(searchHit.getSourceAsString());
        }
